...
export default function TablePage() {
      ...
      if (!initDone) {
        try {
          ...
          addMessage({
            id: 'greet-fallback',
            role: 'assistant',
            content: `Hey! I'm Zara 👋 Welcome to Table ${tableId}. What are you in the mood for?`,
  ...
  return (
    <div className="min-h-screen bg-stone-100 pb-24">
      <header className="sticky top-0 z-30 flex items-center justify-between border-b border-stone-200 bg-white px-4 py-3 shadow-sm">
        <div>
          <h1 className="font-bold text-stone-900">Table {tableId}</h1>
          <p className="text-xs text-stone-500">Smart Dining</p>
        </div>
        <button
          onClick={() => setCartOpen(true)}
          className="relative flex h-11 items-center gap-2 rounded-xl bg-amber-600 px-4 font-semibold text-white min-h-[44px]"
        >
          🛒 Cart
          {totalItems() > 0 && (
      ...
      <main className="space-y-4 p-4">
        ...
        {lastSuggestions.length > 0 && (
          <section>
            <h2 className="mb-2 font-semibold text-stone-800">⭐ AI Pick for You</h2>
      ...
      <button
        ...
        className="fixed bottom-6 right-4 z-40 flex h-14 w-14 items-center justify-center rounded-full bg-amber-600 text-2xl text-white shadow-lg hover:bg-amber-700 min-h-[56px] min-w-[56px]"
